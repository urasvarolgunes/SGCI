...
def test_KNN(model, features, labels, y, idx_test, idx_train=None, N=[2,5,8,10,15,20,30]):
    '''to test for train+valid set, provide idx_train and set idx_test=idx_valid''' 
    model.eval()
    with torch.no_grad():
        pred = model(features, idx_test)

        if idx_train is None: #evaluating on test set
            X = torch.cat((labels, pred), dim=0)
        else:
            X = torch.cat((labels[idx_train], pred), dim=0)
            y = y[torch.cat([idx_train, idx_test]).cpu().numpy()] #idx_test=idx_valid in this case
...
def run_training(fold, params, testing=False, use_all_train=False):
    ...
    best_loss, test_knn_result = None, None
    if not use_all_train:
        val_knn_result = test_KNN(model, features, labels, y, idx_test=idx_valid, idx_train=idx_train, N=[10])
...
if __name__ == "__main__":
    ...
    for fold in range(args.num_folds):
        temp_loss, result = run_training(fold, test_params, testing=True, use_all_train=True)
        test_results.append(result)
    
    print(f'KNN RESULTS FOR {args.model}, {args.base}')
